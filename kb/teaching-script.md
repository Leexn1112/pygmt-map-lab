...
- 作業兩週後繳交，提交 GitHub repository 網址。
...
依序給三張可直接執行的圖：平面彩色地形、加上陰影、再加等高線。每張各有程式 cell 與 API 參考，先跑圖再改參數；後面才接 3D 與旋轉拉桿。

- 使用 `load_earth_relief()` 取得資料、`grdimage()` 繪圖。
- 說明解析度、色階與陰影的用途；全球圖先用較粗資料，區域圖再細化。
- 練習：查 API 找可用解析度、陰影方向、等高線疏密與高度標註設定；每次改一個地方、比較結果，不預先給修改答案。
- AI 輔助：地形部分先有靜態版本、手動試參數；若反覆修改太麻煩，再讓學生請 AI 把陰影角度、等高線間距或 3D 視角做成拉桿。不提供固定提示詞，讓他們決定控制項；改照光或視角時沿用網格，不必重新下載。
...
作業兩週後繳交，只要滿足兩個條件：作品與 PyGMT 有關，以及將作品上傳 GitHub、繳交教師可開啟的 repository 連結。
